...
- **YAML as source of truth**: Device type and subtype live in the YAML config. Cover inversion can be forced with `invert_position`, otherwise the controller falls back to the learned device profile. A battery/solar device that sleeps to save power is declared with `low_power: true` so directed commands wake it with a long radio preamble; the default (`false`) suits mains-powered, always-listening devices. Pairing prints a ready-to-paste YAML snippet in the logs when enough metadata is known.
...
- `low_power` (Optional, default `false`): Declare this device as a low-power / duty-cycled receiver — a battery or solar actuator (Velux INTEGRA SOLAR blinds, Somfy solar/battery motors) that sleeps to save power and must be woken with a long radio preamble. Directed commands to it then set the `CTRL1_LOW_POWER` flag and use the 1024-byte wake-up preamble. Leave it unset for mains-powered, always-listening devices (the majority) — sending those the long preamble can stop them hearing the command at all. Pairing and `scan_paired_devices` pre-fill this line in the ready-to-paste snippet from the device's own reported power class.
