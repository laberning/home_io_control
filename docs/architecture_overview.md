...
- [Protocol Layer](https://laberning.github.io/home_io_control/group__hioc__protocol.html): frame layout, command builders, cryptographic helpers, and shared protocol utilities. The protocol model is split into cohesive headers (`proto_sizes.h`, `proto_timing.h`, `proto_constants.h`, `proto_device_model.h`, `proto_codecs.h`, `proto_commands.h`, `proto_crypto.h`) with `proto_frame.h` holding only the frame container; each header's includers name their real dependencies directly rather than relying on an umbrella.
- [Radio Driver Layer](https://laberning.github.io/home_io_control/group__hioc__radio.html): the `RadioDriver` abstraction and the SX1276 / SX1262 / LR1121 implementations. Chips without hardware IoHomeOn framing (SX1262 and LR1121) share the software PHY in `radio_soft_phy.h/.cpp` (UART bit-encoding for TX, UART-decode probe with CRC validation for RX) and the IRQ-driven RX/TX orchestration in `SoftPhyDriverBase` (`radio_soft_phy_driver_base.h/.cpp`), which `RadioSX1262`/`RadioLR1121` both derive from — SPI opcode encoding/transport and IRQ bit values/word width stay behind virtual primitives each chip implements. The LR1121 driver is hardware-validated: authenticated 2W exchanges (open/close/stop with real position/state feedback) confirmed against a real Somfy Sunea IO awning motor.
- [Controller Layer](https://laberning.github.io/home_io_control/group__hioc__hub.html): `IOHomeControlComponent` orchestrates setup and loop scheduling through ten collaborator objects (eleven when `lr1121_firmware_update:` is configured) — `ExchangeEngine` (authenticated exchanges, and the shared `listen()` primitive every radio wait — its own and `PairingEngine`'s — runs through), `PairingEngine` (discovery and pairing), `ManagementActions` (rename/identify/force-open/scan-paired-devices actions), `DeviceRegistry` (device table and callbacks), `OperationQueue` (pending-operation coalescing), `StatusPollPolicy` (per-device poll scheduling), `PairingTelemetry` (per-attempt pairing event recorder, shared with both engines; its read-only companion `pairing_advisor.h` turns a completed attempt into actionable diagnostics), `OneWayTransmitter` (1W command bursts — the one radio-driving collaborator that awaits nothing), `KeyExtractionResponder` (the impure half of the "Accept Foreign Pairing" feature), `OnewayKeyAdoption` (receive-only 1W controller-key adoption), and — compile-gated — `Lr1121FirmwareUpdateController` (LR1121 flash orchestration). The hub itself is split by concern: `hub_core.cpp` (lifecycle and loop), `hub_operations.cpp` (queued operation dispatch), `hub_status.cpp` (passive receive-side handling), plus a thin `hub_pairing.cpp` wrapper around `PairingEngine` (the `ManagementActions` and collaborator forwarders are inline one-liners on `IOHomeControlComponent`). `hub_decisions.h` holds pure frame-classification helpers testable without radio or timing. The "Accept Foreign Pairing" key-extraction responder is `PairingEngine`'s device-role mirror: where `PairingEngine` drives the hub *pairing to* a device (blocking, one call per attempt), `pairing_responder.h/.cpp` is event-driven and stateless-per-call — its pure ARMED_IDLE→SENT_DISCOVER_RESP→SENT_CONFIRM_ACK→SENT_CHALLENGE→EXTRACTED transitions are invoked from the 0x28/0x2C/0x31/0x32 branches inside `process_received_packet_()` (`hub_status.cpp`) while the hub keeps operating normally; the impure side (arming, throwaway node-ID generation, the auto-off timer, and the security log block) lives in the `KeyExtractionResponder` collaborator (`key_extraction_responder.h/.cpp`). Hardware-confirmed end to end against a VELUX KLR200 and a Somfy TaHoma — see [Key extraction](key-extraction.md).
- [Tuning Layer](https://laberning.github.io/home_io_control/group__hioc__tuning.html) (sub-group of the Controller Layer): the runtime `TuningConfig`, the table-driven `tuning_registry`, and the optional Home Assistant number/select entities.
- [ESPHome Integration Layer](https://laberning.github.io/home_io_control/group__hioc__entities.html): the runtime entities plus the Python schema/codegen modules that expose the component to ESPHome. Shared device-binding codegen lives in `platform_common.py`; the C++ counterparts are three mixins in `platform_entity_base.h` — `DeviceBoundEntity` for the main entity platforms, `DeviceBoundCompanion` for the per-device entities that do not own the device (the auto-generated companion diagnostic sensors — device name, active issue, RSSI, last contact, exchange failures, last commanded by, last command source — and the cover command buttons — favorite/vent — plus the silent-operation switch), and `HubBoundEntity` for the hub-level control entities that need only a parent pointer. The tuning number/select entities keep their own constructor-injected parent to avoid pulling `hub_core.h` into their header.
...
1. A YAML declaration is validated by the Python code-generation modules in [Python Code Generation](https://laberning.github.io/home_io_control/group__hioc__codegen.html).
2. ESPHome codegen wires those declarations to runtime C++ objects such as `IOHomeControlComponent`, `IOHomeCover`, `IOHomeLight`, `IOHomeLock`, `IOHomeSwitch`, and the generated companions (`IOHomeDeviceNameTextSensor` per entity, `IOHomeDiscoverButton` and its `IOHomePairingResultTextSensor` when `home_io_control.discover_and_pair_button: true`, `IOHomeAcceptForeignPairingSwitch` when `home_io_control.accept_foreign_pairing: true`, `IOHomeScanPairedDevicesButton` when `home_io_control.scan_paired_devices_button: true`, `IOHomeLr1121FirmwareUpdateButton` when `home_io_control.lr1121_firmware_update:` is configured — hub-level options, not `switch:`/`button:` platform entries, so none can ever be confused with a device-bound `IOHomeSwitch`/`IOHomeCoverCommandButton` and each compiles to nothing (the LR1121 one behind `IOHOME_LR1121_FIRMWARE_UPDATE`) when its option is absent).
3. Runtime entities call into the hub through the high-level operation methods documented in [Controller Layer](https://laberning.github.io/home_io_control/group__hioc__hub.html).
4. The hub builds protocol frames using helpers from [Protocol Layer](https://laberning.github.io/home_io_control/group__hioc__protocol.html) and sends them through a concrete radio backend from [Radio Driver Layer](https://laberning.github.io/home_io_control/group__hioc__radio.html).
...
The device-name feature has separate read and write paths that share the same cached UTF-8 registry field.

## Home Assistant Action Surface

Hub-level operations that should not become permanent entities are exposed through ESPHome native API actions: `rename_device`, `identify_device`, `force_open_device`, and `scan_paired_devices`. All four share one data-driven descriptor class (`detail::ManagementServiceDescriptor` in `management_actions.cpp`), parametrized by action name, argument list, and callback — adding action N+1 is a registration call, not a new class. `scan_paired_devices` takes no arguments — it broadcasts a roll-call and reports every device that answers, rather than acting on one target device.

`IOHomeControlComponent` registers each descriptor with `api::APIServer` through its
`ManagementActions` collaborator, which then emits `esphome.home_io_control_action_result` so an
automation can inspect success and verification details. Home IO Control enables the required
native API compile-time flags internally, so user YAML only needs a normal `api:` block.

How to call each action from Home Assistant, with its fields and result event, is in
[Home Assistant actions](configuration/actions.md).
...
5. Name-request failures are isolated from movement/status behavior; unsupported devices keep an empty cached name.
...
A versioned corpus of real captured IO-Homecontrol frames lives at
`tests/corpus/captures/<phase>/<id>.yaml`, one YAML file per scenario, `filename == id` and
`id = <subject>_<phase>[_<scenario>][_<chip>]` (naming convention + contribution workflow in
`tests/corpus/README.md`; the closed phase vocabulary and registered subject set are
`scripts/corpus/naming.py`). `scripts/corpus/build.py` renders the
...
CRC/CTRL0 self-consistency, cryptographic promises, the naming convention (`filename == id`,
...
- The user documentation starts at the [documentation index](index.md); this page is the bridge into the API reference.
