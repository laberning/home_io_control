...
`GENERATE_TODOLIST` and `GENERATE_BUGLIST` are enabled, but plain `// TODO:` comments are not enough to populate those pages. They need to be written as Doxygen `\todo` / `\bug` commands, otherwise the generated lists stay empty.