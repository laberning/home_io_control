...
/// failure. The ACK constant is retained for inbound frame parsing and logging, and for three
...
/// tuning knob (default off, tuning_config.h); the SPE roll-call's low-power frame shape
/// (`CTRL1 = LOW_POWER | ACK`) sets it unconditionally, matching the header a real VELUX hub uses
/// for its own roll-call; and create_discover_confirm()'s `ack` parameter, opt-in and gated by the
/// `pairing_discover_confirm` tuning knob (`send_with_ack`, default off).
