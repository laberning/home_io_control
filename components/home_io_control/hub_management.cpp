...
/// @ingroup hioc_hub
///
/// This file owns advanced management operations that are not part of the normal cover/light/
/// switch entity surface. These operations are exposed as ESPHome native API actions so Home
/// Assistant can trigger them on demand without adding always-visible helper entities.
...
/// This keeps the Home Assistant action surface identical to native ESPHome actions while
/// avoiding the unresolved link path behind CustomAPIDevice::register_service().
class RenameDeviceServiceDescriptor : public api::UserServiceDescriptor {
 public:
  explicit RenameDeviceServiceDescriptor(IOHomeControlComponent *parent)
      : parent_(parent), key_(fnv1_hash(MANAGEMENT_ACTION_RENAME_DEVICE)) {}
  ...
  bool execute_service(const api::ExecuteServiceRequest &request) override {
    ...
    this->parent_->api_rename_device_(request.args[0].string_.str(), request.args[1].string_.str());
    return true;
  }
...
IOHomeControlComponent::ManagementActionResult make_management_result(const std::string &action,
                                                                      const std::string &device_id) {
  IOHomeControlComponent::ManagementActionResult result;
  result.action = action;
  result.device_id = device_id;
  return result;
}

void IOHomeControlComponent::register_management_actions_() {
#if defined(USE_API_USER_DEFINED_ACTIONS) && defined(USE_API_CUSTOM_SERVICES)
...
void IOHomeControlComponent::publish_management_result_(const ManagementActionResult &result) {
  if (result.success) {
  ...
  this->fire_homeassistant_event(MANAGEMENT_RESULT_EVENT, event_data);
}

void IOHomeControlComponent::api_rename_device_(const std::string &device_id, const std::string &new_name) {
  this->publish_management_result_(this->rename_device(device_id, new_name));
}
...
IOHomeControlComponent::ManagementActionResult IOHomeControlComponent::rename_device(const std::string &device_id,
  ...
  auto *dev = this->get_device(normalized_device_id);
  if (dev == nullptr) {
  ...
  IoFrame request;
  if (!create_set_name(request, this->node_id_, dev->node_id, payload)) {
  ...
  if (!this->send_and_receive_(request, response, FREQ_CH2)) {
    this->log_exchange_debug_(normalized_device_id.c_str());
  ...
  if (!this->request_device_name(normalized_device_id)) {
    result.message = "rename acknowledged but verification readback failed";
    return result;
  }

  auto *updated_device = this->get_device(normalized_device_id);
